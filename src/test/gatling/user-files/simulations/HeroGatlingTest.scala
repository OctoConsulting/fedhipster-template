...
/**
 * Performance test for the Hero entity.
 */
class HeroGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Hero entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all heroes")
            .get("/api/heroes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new hero")
            .post("/api/heroes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
                , "affilition":"SAMPLE_TEXT"
                , "eyeColor":"SAMPLE_TEXT"
                , "hairColor":"SAMPLE_TEXT"
                , "gender":"SAMPLE_TEXT"
                , "status":"SAMPLE_TEXT"
                , "year":null
                , "firstAppearance":"SAMPLE_TEXT"
                , "universe":"SAMPLE_TEXT"
                , "intelligence":null
                , "strength":null
                , "speed":null
                , "durability":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_hero_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created hero")
                .get("${new_hero_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created hero")
            .delete("${new_hero_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
